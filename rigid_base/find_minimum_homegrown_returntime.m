function [zeq,nsteps,Happrox,all_time] = find_minimum_homegrown_returntime(nseg,zguess,initH,diagflag)
...
% Timers for how long algorithm spends doing various types of work
  ...
  while norm(gradeq) > gradtol
    ...
    if mod(count1+count2+count3,100) == 0
...
function H = set_H_to_inv_of_adjusted_true_hessian(nseg,zeq)    
  ...
  [V,D]=eigs(hesseq,7*(nseg-1)+6*(nseg+1));
  for i = 1:7*(nseg-1)+6*(nseg+1)
      if D(i,i) < 0.001
          D(i,i)=1;
      else
          D(i,i)=1/D(i,i);
